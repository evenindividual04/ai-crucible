...
class CrucibleDisplay:
    ...
    def print_header(self, state: CrucibleState) -> None:
        ...
        # War room mode
        title = Text()
        title.append("THE CRUCIBLE", style="bold cyan")
        title.append(" // ", style="dim")
        title.append("ADVERSARIAL ENGINE", style="cyan")
        
        status_style = STATUS_COLORS.get(state.status, "white")
        info = Text()
        info.append(f"Iteration: {state.iteration_count}/{state.max_iterations} | ")
    ...
    def print_summary_table(self, state: CrucibleState) -> None:
        ...
        for vuln in state.vulnerabilities:
            ...
            outcome = "PATCHED" if vuln.vulnerability_id in patched_ids else "UNRESOLVED"
            outcome_style = "green" if outcome == "PATCHED" else "yellow"
            ...
            table.add_row(
                ...
                vuln.title[:40] + ("..." if len(vuln.title) > 40 else ""),
